...
public class SimpleDomainEventStream implements DomainEventStream {
	...
	/**
	 * Initialize the event stream using the given List of DomainEvent and
	 ...
	 */
	public SimpleDomainEventStream(final Collection<? extends DomainEvent> events) {
		this(events.toArray(new DomainEvent[events.size()]));
	}

	/**
	 * Initialize the event stream using the given
	 * {@link org.axonframework.domain.DomainEvent}s and aggregate identifier.
	 * The aggregate identifier is initialized by reading it from the first
	 * event available. Therefore, you must provide at least one event.
	 * 
	 * @param events
	 *            the list of domain events to stream
	 * @throws IllegalArgumentException
	 ...
	 */
	public SimpleDomainEventStream(final DomainEvent... events) {
	...
	/**
	 * Append the contents of this stream to the given <code>collection</code>.
	 * This includes all events that have already been read from the stream.
	 * 
	 * @param collection
	 *            the collection to append the events to.
	 */
	public void appendTo(final Collection<DomainEvent> collection) {
		collection.addAll(Arrays.asList(events));
	}

	/**
	 * Creates an empty stream. For performance reasons, this method always
	 * returns the same instance.
	 * 
	 * @return en empty DomainEventStream
	 */
	public static DomainEventStream emptyStream() {
		return SimpleDomainEventStream.EMPTY_STREAM;
	}
}
